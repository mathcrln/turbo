...
// computations. #[derive(Clone, Default)]
// struct Version(Arc<()>);
...
impl FileHashes {
    ...
    // fn get_package_path(&self, file_path: &AnchoredSystemPath) ->
    // Option<&AnchoredSystemPath> {     self.0
    //         .get_ancestor(file_path.as_str())
    //         .and_then(|subtrie| subtrie.key())
    //         .map(|package_path| {
    //             AnchoredSystemPath::new(package_path).expect("keys are valid
    // AnchoredSystemPaths")         })
    // }
